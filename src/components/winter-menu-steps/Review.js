...
import Table from '../table/TableWinterMenu';
...
const Review = ({
  ...
  useEffect(() => {
    ...
    const header = Object.keys(winterMenus[0]);
    const winterMenuCsv = winterMenus.map((row) => {
      //   console.log(row);
      return header
        .map((fieldName) => {
    ...
    const addOnCsv =
      addOns.length > 0
        ? addOns
            ...
            .join('\r\n')
        : '';
    const csv = [...winterMenuCsv, addOnCsv].join('\r\n');
    // console.log(csv);
    setSubmittedCust(csv);
    // console.log(customizations);
    // console.log(submittedCust);
  }, [winterMenus, addOns, submittedCust]);
