...
export const CardListWinterMenu = ({
  groups,
  handleSelect,
  hasNone,
  stateKey,
  ...
  const foodItemRow =
    ...
    groups.map((group) => (
      <CardItemNumber
        key={group.id}
        group={group}
        itemsRemaining={form.winterMenusRemaining}
        handleItemsChange={form.handleWinterMenuChange}
      />
    ));
  return (
    <div className='section__item'>
      <h2 className='section__heading'>{`Select your ${groupName}s`}</h2>
      <p className='section__subheading'>{`Select your ${selectedPackage?.acf?.meal_count} items from our winter menu options (required)`}</p>
      <div className='section__grid grid-x grid-margin-x align-center'>{foodItemRow}</div>
    </div>
  );
};

export default CardListWinterMenu;
