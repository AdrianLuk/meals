...
const Customize = ({
  ...
  const customization = useMemo(() => {
    return {
      ...
      // selectedMeat,
      meat: isEmptyObject(selectedFallMenu) && !isEmptyObject(selectedMeat) ? meatVariant : {},
      // selectedCarb,
  ...
  useEffect(() => {
    ...
    if (isEmptyObject(selectedFallMenu)) {
      total =
        (carbVariant?.extra_charge ? +carbVariant.extra_charge * customizationCount : 0) +
        (meatVariant?.extra_charge ? +meatVariant.extra_charge * customizationCount : 0) +
  ...
  return (
    <section className='section section--customize'>
      ...
      {isEmptyObject(selectedMeat) && isEmptyObject(selectedCarb) && isEmptyObject(selectedVeg) && (
        <CardList
          groupName='menu'
          stateKey='selectedFallMenu'
          heading={`Fitaxx menu`}
          subheading={`No substitutions allowed.`}
      ...
      <div className='section__item'>
        ...
        <p className='section__subheading'>
          <strong>We also serve halal meat!</strong> Please add any comments for this customization
        </p>
        <p className='section__subheading'>
          <em>
            Please Note: Items in salad can be removed but not substituted. For more information,{' '}
            <a href={`${formContext.baseURL}/frequently-asked-questions`}>click here</a>
          </em>
        </p>
